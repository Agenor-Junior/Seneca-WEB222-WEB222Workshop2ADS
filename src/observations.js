...
/*******************************************************************************
 * Problemart 1: 01 P use a for-loop to iterate over Arrays
 *
...
function observationsByPrivacy(data, privacy) {
  ...
  // Normalize the privacy value to lowercase if it's a string
  const normalizedPrivacy = typeof privacy === 'string' ? privacy.toLowerCase() : privacy;

  // Check if the privacy value is valid
  if (!validPrivacyValues.includes(normalizedPrivacy)) {
  ...
  for (const observation of data.results) {
    if (observation.privacy === normalizedPrivacy) {
